...
/*For yla_number_type(double) */
static int test_push_pull_numbers()
{
    yla_stack stack;
    size_t stack_size = 3;
    yla_stack_init(&stack, stack_size);
    yla_number_type vals[] = {3.22, 314, 66.6, 99};
    ...
    YLATEST_ASSERT_TRUE(yla_stack_push(&stack, vals[0]), "stack with deep 3 could able to push 1 value");
    ...
    YLATEST_ASSERT_TRUE(yla_stack_push(&stack, vals[1]), "stack with deep 3 could able to push 2 value");
    YLATEST_ASSERT_TRUE(yla_stack_push(&stack, vals[2]), "stack with deep 3 could able to push 3 value");

    YLATEST_ASSERT_TRUE(yla_stack_is_full(&stack), "stack with deep 3 must be full after 3 push");
    YLATEST_ASSERT_FALSE(yla_stack_push(&stack, vals[3]), "stack with deep 3 could NOT able to push 4 value");
    YLATEST_ASSERT_TRUE(yla_stack_is_full(&stack), "stack with deep 3 must be full after 3 push");

    YLATEST_ASSERT_TRUE(yla_stack_pull(&stack, &result), "stack with deep 3 could able to pull 1 value");
    YLATEST_ASSERT_TRUE(result==vals[2], "incorrect value after 1 pull");

    YLATEST_ASSERT_TRUE(yla_stack_pull(&stack, &result), "stack with deep 3 could able to pull 2 value");
    YLATEST_ASSERT_TRUE(result==vals[1], "incorrect value after 2 pull");

    YLATEST_ASSERT_TRUE(yla_stack_pull(&stack, &result), "stack with deep 3 could able to pull 3 value");
    YLATEST_ASSERT_TRUE(result==vals[0], "incorrect value after 3 pull");
...
static int test_deep_n_numbers()
{
    yla_stack stack;
     size_t stack_size = 3;
    yla_stack_init(&stack, stack_size);
    ...
    yla_stack_push(&stack, vals[0]);

    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 0, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[0], "yla_stack_get_deep returns incorrect value for index 0");

    YLATEST_ASSERT_FALSE(yla_stack_get_deep(&stack, 1, &result), "yla_stack_get_deep must return OK if stack deep OK");


    yla_stack_push(&stack, vals[1]);

    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 0, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[1], "yla_stack_get_deep returns incorrect value for index 0");

    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 1, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[0], "yla_stack_get_deep returns incorrect value for index 1");

    YLATEST_ASSERT_FALSE(yla_stack_get_deep(&stack, 2, &result), "yla_stack_get_deep must return OK if stack deep OK");


    yla_stack_push(&stack, vals[2]);

    YLATEST_ASSERT_FALSE(yla_stack_get_deep(&stack, 100, &result), "yla_stack_get_deep must return error if stack deep exceeded");

    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 0, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[2], "yla_stack_get_deep returns incorrect value for index 0");

    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 1, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[1], "yla_stack_get_deep returns incorrect value for index 1");

    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 2, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[0], "yla_stack_get_deep returns incorrect value for index 2");

    YLATEST_ASSERT_FALSE(yla_stack_get_deep(&stack, 3, &result), "yla_stack_get_deep must return error if stack deep exceeded");


    YLATEST_ASSERT_FALSE(yla_stack_set_deep(&stack, 100, vals[3]), "yla_stack_set_deep must return error if stack deep exceeded");

    YLATEST_ASSERT_TRUE(yla_stack_set_deep(&stack, 0, vals[2]), "yla_stack_set_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 0, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[2], "yla_stack_get_deep returns incorrect value for index 0");

    YLATEST_ASSERT_TRUE(yla_stack_set_deep(&stack, 1, vals[1]), "yla_stack_set_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 1, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[1], "yla_stack_get_deep returns incorrect value for index 1");

    YLATEST_ASSERT_TRUE(yla_stack_set_deep(&stack, 2, vals[0]), "yla_stack_set_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(yla_stack_get_deep(&stack, 2, &result), "yla_stack_get_deep must return OK if stack deep OK");
    YLATEST_ASSERT_TRUE(result==vals[0], "yla_stack_get_deep returns incorrect value for index 2");

    YLATEST_ASSERT_FALSE(yla_stack_set_deep(&stack, 3, vals[2]), "yla_stack_get_deep must return error if stack deep exceeded");
...
YLATEST_BEGIN(yla_stack_test)
  ...
  YLATEST_ADD_TEST_CASE(test_deep_n_numbers)
  YLATEST_ADD_TEST_CASE(test_push_pull_numbers)
YLATEST_END
